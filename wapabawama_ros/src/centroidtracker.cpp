...
std::vector<std::pair<int, std::pair<int, int>>> CentroidTracker::update(vector<darknet_ros_msgs::BoundingBox> boxes) {
    if (boxes.empty()) {
    ...
    for (auto& box : boxes) {
        int cX = int((box.xmin + box.xmax) / 2.0);
        int cY = int((box.ymin + box.ymax) / 2.0);
